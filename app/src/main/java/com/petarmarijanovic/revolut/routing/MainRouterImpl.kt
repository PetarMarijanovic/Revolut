...
class MainRouterImpl(
    ...
    override fun showRates() {
        fragmentManager.inTransactionAndAddToBackStack(RatesFragment.TAG) {
            add(MAIN_CONTAINER_ID, RatesFragment.newInstance(), RatesFragment.TAG)
        }
    }

    override fun showNextScreen() {
        fragmentManager.inTransactionAndAddToBackStack(NextScreenFragment.TAG) {
            setCustomAnimations(R.anim.fragment_right_enter, R.anim.nothing, R.anim.nothing, R.anim.fragment_right_exit)
            add(MAIN_CONTAINER_ID, NextScreenFragment.newInstance(), NextScreenFragment.TAG)
        }
    }
